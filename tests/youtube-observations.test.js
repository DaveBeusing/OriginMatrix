...
test("classifies ad evidence without treating absence as blocked", () => {
  ...
  assert.equal(classifyAdObservation({ detected: true, visible: false, originMatrixHidden: true }), OBSERVATION_STATUS.SUPPRESSED);
  assert.equal(classifyAdObservation({ detected: true, visible: true, originMatrixHidden: false }), OBSERVATION_STATUS.VISIBLE);
  assert.equal(classifyAdObservation({ detected: true, visible: false, originMatrixHidden: false }), OBSERVATION_STATUS.OBSERVED);
