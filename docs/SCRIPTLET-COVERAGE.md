...
OriginMatrix 1.24.0 analyzes every enabled filter list plus My Filters before expanding executable scriptlets. The development CLI inventories all six bundled snapshots so compatibility work can be ranked before opt-in lists are enabled. Run it for any hostname with:
