...
Advanced network modifiers remain normalized and browser-independent until compilation. `$removeparam=` becomes a DNR query transform for literal parameter names; `$to=` and `$denyallow=` become request-domain conditions; `$method=` becomes request-method inclusion or exclusion; and `$match-case` enables case-sensitive URL matching. The parser also normalizes `1p`/`3p`, `doc`, and `frame` aliases. Regex and negated parameter transformations, parameter-removal exceptions, and contradictory destination intersections are rejected instead of approximated.
...
The versioned `FilterListSettingsStore` retains catalog defaults and user choices across service-worker restarts. Toggling any list rebuilds the unified candidate from the remaining enabled sources. State is persisted only after successful activation, and a failed toggle restores the previous combined generation.

`FilterListUpdater` and the versioned `FilterListGenerationStore` apply the same controls independently to every catalog entry. Updates use only catalogued HTTPS URLs, omit credentials, reject redirects, cap declared and actual payload size at 5 MB, require either an Adblock header or official title metadata, require Version or parseable Last-modified metadata and a meaningful rule count, then calculate SHA-256. Parsing, unified compilation, cosmetic preparation, scriptlet validation, and shared rule-budget checks all occur before the candidate can replace the active generation. The downloaded source is stored only after successful activation; activation or storage failure reinstalls the previous combined generation.

Network deduplication excludes source attribution from its semantic signature. Identical rules across lists therefore compile once while their sorted source-list and original-rule attributions remain attached to the resulting DNR rule ID.
...
`ScriptletRegistry` is an immutable allowlist of bundled implementation functions. Each definition owns its canonical name, aliases, immutable EARLY or NORMAL phase, argument schema, and function reference. Seven implementations cover bounded constant/storage changes, property-read/write aborts, literal JSON property pruning, attribute removal, and text removal. Unsafe property paths, arbitrary expressions, wildcard JSON paths, oversized text, and unsupported selectors are rejected. Filter data cannot register implementations.

`ScriptletParser` accepts the selected uBlock-style `domain##+js(name, args...)` form, including included/excluded domains and bounded quoted or escaped arguments. Alias ownership belongs to the registry rather than the syntax parser. Global rules, exceptions, malformed calls, excessive arguments, and wider dialects are reported as unsupported. Parsed names still pass through the closed registry, so unknown identifiers and invalid arguments never become active.
...
Across all six bundled snapshots, Phase 4 measures 1,153 of 3,286 scriptlet references as supported (35.1%). Exactly one of 659 conservatively YouTube-relevant references passes both parser and registry validation; unsupported wildcard-domain observations are not promoted to executable rules. See `SCRIPTLET-COVERAGE.md` for the measured ranking and interpretation.

The common `set`, `rmnt`, `aopr`, `aopw`, and `ra` names resolve through registry aliases to reviewed implementations. Constant values remain an explicit allowlist. `json-prune` accepts only bounded literal paths and installs its JSON hook only for a matching active rule. Attribute persistence uses a targeted observer only when the validated `stay` behavior is requested. Higher-risk `acs`, Fetch/XHR, cookie, wildcard JSON, and trusted response-edit primitives remain unsupported pending narrowly testable semantics.

`ScriptletEngine` validates a filter-list generation before activation, resolves aliases, applies domain inclusions/exclusions per document, filters and deduplicates canonical invocations by phase, and brands plans through the registry. Property and JSON hooks are EARLY; bounded DOM mutation scriptlets are NORMAL. The `document_start` content script requests EARLY execution before other engine messages and waits for it before requesting NORMAL execution after DOM readiness. The service worker derives hostname, tab, frame, and document identity only from Chrome's sender metadata and deduplicates each phase per document. `execute` accepts only branded plans and passes the already-bundled function reference plus validated string arguments to `chrome.scripting.executeScript` in the `MAIN` world. No code string, `eval`, `Function`, remote script, or dynamically downloaded implementation is accepted.
