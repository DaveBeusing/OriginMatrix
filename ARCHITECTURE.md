...
`FilterListManager` coordinates list services with the versioned `FilterListSettingsStore`. Enabled state survives service-worker restarts. Disabling a list removes its DNR partition and clears its cosmetic, scriptlet, and automatic-attribution generations together; enabling rebuilds them from the packaged source. State is persisted only after successful activation, and a failed toggle restores the previous generation. Phase 15 intentionally manages only catalogued bundled lists—remote update, custom URL, and local import require Phase 16's validation and rollback pipeline.
