...
- Allowlisted bundled Scriptlet Engine with controlled MAIN-world execution
...
Automatic protection combines the small static proof ruleset with a pinned EasyList snapshot. EasyList network filters and simple cosmetic selectors are parsed locally as data; unsupported syntax is reported and never guessed. Cosmetic selectors are indexed by site and injected only on matching documents. Batched DOM observation handles inserted elements and class/ID changes without rescanning every mutation individually. A secure Scriptlet Engine now provides three bundled implementations, but filter-list scriptlet syntax is not parsed or activated yet. Runtime filter downloads and updates, procedural cosmetic filters, uMatrix text-rule conversion, a Relaxed tracker-list profile, and Public-Suffix-List-based domain grouping are not currently included.
...
- Selected scriptlet-filter syntax and argument parsing
